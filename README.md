...
- `gpu-usage` (also accepts legacy `gpu_usage`)
  ...
  - `temp-read` (`"drm"` or `"sysfs"`, default: `"drm"`): where the GPU temperature is read from. `"drm"` uses the `AMDGPU_INFO_SENSOR_GPU_TEMP` ioctl and keeps a DRM device handle open for the lifetime of the process. `"sysfs"` reads the amdgpu hwmon `temp1_input` attribute instead, so no DRM handle is opened for temperature. Both report the same sensor: the kernel defines `AMDGPU_PP_SENSOR_EDGE_TEMP = AMDGPU_PP_SENSOR_GPU_TEMP`, so the ioctl and `temp1_input` resolve to the same value. Use `"sysfs"` to avoid holding a DRM client open; after five consecutive failed reads it hands over to the DRM ioctl for the rest of the run, which does open one.
