...
- `gpu-usage` (also accepts legacy `gpu_usage`)
  ...
  - `temp-read` (`"drm"` or `"sysfs"`, default: `"drm"`): where the GPU temperature is read from. `"drm"` uses the `AMDGPU_INFO_SENSOR_GPU_TEMP` ioctl and keeps a DRM device handle open for the lifetime of the process. `"sysfs"` reads the amdgpu hwmon `temp1_input` attribute instead, so no DRM handle is opened for temperature. Both report the same sensor: the kernel defines `AMDGPU_PP_SENSOR_EDGE_TEMP = AMDGPU_PP_SENSOR_GPU_TEMP`, so the ioctl and `temp1_input` resolve to the same value. Use `"sysfs"` to avoid holding a DRM client open; combined with `method = "kernel"` or `"process"` the governor then opens no DRM device at all. The hwmon attribute is plain upstream amdgpu and is present on a stock kernel, but `"sysfs"` degrades safely anyway: if the attribute is missing, unreadable, not an integer, or reports an implausible temperature, the governor logs a warning and uses the DRM ioctl instead. At startup that happens on the first failed probe. Mid-run it takes five consecutive failures: the temperature only drives thermal throttling, so an isolated error is ridden out by reusing the last good reading rather than by giving up on the source, since falling back reopens the DRM handle that `"sysfs"` exists to avoid. Each source is a `TempStrategy` implementation, alongside the existing `UsageStrategy` and `FreqStrategy`; the sysfs one lives in `src/gpu/sysfs_temp_strategy.rs`.
